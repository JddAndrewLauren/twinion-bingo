...
| Admin — room list | `/admin`, an operator's phone (#125) | **`phone` is the primary viewport for this screen, not the iPad** — the first surface this file pins to a non-default one. **Locked**: only the secret form on screen, no room data anywhere in the DOM, whether no secret was tried yet or the wrong one was — a denial reads as generic ("Wrong secret.") rather than naming whether any room exists. **Unlocked**: the list — code, theme, roster, game state, age — fits at `phone` without scrolling sideways, the secret entry field and Unlock button both thumb-sized (44px), and the list refreshing on its own on a timer rather than needing a manual reload |
| Admin — mutating actions | `/admin`, unlocked, at `phone` (#126) | Each room row's **End game** (live rooms only), **Delete** and per-player **Kick** buttons are reachable without scrolling the row out of the viewport's width; each opens a native `confirm()` before sending anything, and a declined confirm sends no request. A busy row's own buttons read a verb-ing label (**Ending…**, **Deleting…**, **Kicking…**) while their request is out. Ending a room's game with no room to end, or kicking a player no longer in the room, fails without breaking the list on screen |
